...
export default function ReviewerProfile() {
  const { user } = useAuth();
  const initials = user?.name?.split(' ').map(n => n[0]).join('').slice(0, 2) || 'RV';
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-3xl">
      <PageHeader title="Reviewer Profile" subtitle="Manage your reviewer information" icon={UserCircle} />

      {/* Avatar & Basic Info */}
      <div className="bg-card rounded-xl border border-border shadow-card p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-purple-500 to-purple-600 text-white text-xl font-bold shadow-lg">
              {initials}
            </div>
            <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-card rounded-lg border border-border p-4">
          <p className="text-xs font-semibold text-muted-foreground">Reviewer Status</p>
          <p className="text-sm font-bold text-purple-600 mt-1">Active</p>
        </div>
        <div className="bg-card rounded-lg border border-border p-4">
          <p className="text-xs font-semibold text-muted-foreground">Review Access</p>
          <p className="text-sm font-bold text-purple-600 mt-1">Enabled</p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {isEditing && (
            <div className="flex gap-2">
              <Button
                className="bg-purple-600 text-white border-0 hover:bg-purple-700 h-8 text-sm"
        ...
        <div className="space-y-4">
          ...
          {isEditing && (
            <div className="flex gap-3 pt-3 border-t border-border">
              <Button
                className="flex-1 bg-purple-600 text-white border-0 hover:bg-purple-700 h-9"
                onClick={handleSaveProfile}
                disabled={isSaving}
              >
