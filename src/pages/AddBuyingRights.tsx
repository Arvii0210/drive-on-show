...
const AddOrEditAgreement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const {
      personId, role, advancePaid, advancePaidDate, royalties,
      language, country, currency, active
    } = formData;

    if (!personId || !role || !advancePaid || !advancePaidDate || !royalties || !language || !country) {
      toast({
    ...
    // Remove unwanted fields from payload before sending to backend
    const fieldsToRemove = ['state'];
      ...
      <Card className="max-w-5xl mx-auto p-6 md:p-8 space-y-6 shadow-md">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={formData.role} onValueChange={val => handleChange("role", val)}>
                ...
                <SelectContent>
                  {["proprietor", "agency", "author", "publisher", "translator", "editor"].map(role => (
            ...
            <div className="space-y-1.5">
              ...
              <Select value={formData.personId} onValueChange={val => handleChange("personId", val)} disabled={!formData.role || loadingPeople}>
                <SelectTrigger>
                  <SelectValue placeholder={!formData.role ? "Select a role first" : loadingPeople ? "Loading..." : "Select person"} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Input type="number" value={formData.advancePaid} onChange={e => handleChange("advancePaid", e.target.value)} />
            </div>

            {/* Advance Date */}
            <div className="space-y-1.5">
              <Label>Advance Date <span className="text-red-500">*</span></Label>
              <Input type="date" value={formData.advancePaidDate} onChange={e => handleChange("advancePaidDate", e.target.value)} />
            </div>

            {/* Royalties */}
            <div className="space-y-1.5">
              <Label>Royalties (%) <span className="text-red-500">*</span></Label>
              <Input type="number" min="0" max="100" step="0.1" value={formData.royalties} onChange={e => handleChange("royalties", e.target.value)} />
            </div>
          ...
          <div className="flex justify-end gap-4 pt-6 border-t">
            ...
            <Button type="submit" className="bg-blue-600 text-white">{isEdit ? "Update Agreement" : "Add Agreement"}</Button>
          </div>
        </form>
      </Card>
    </div>
