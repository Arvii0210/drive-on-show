...
type Tab = 'author' | 'translator' | 'editor' | 'proprietor' | 'agency' | 'publisher';

const tabs: Tab[] = ['author', 'translator', 'editor', 'proprietor', 'agency', 'publisher'];
...
const People = () => {
  ...
  const [errorMap, setErrorMap] = useState<Record<Tab, string | null>>(() =>
    Object.fromEntries(tabs.map((tab) => [tab, null])) as Record<Tab, string | null>
  );
  ...
  useEffect(() => {
    ...
    const uniqueNames = Array.from(new Set(nameMatches.map(m => m.person.name?.toLowerCase())));
    if (uniqueNames.length === 1 && nameMatches.length > 1) {
      toast({
        title: '⚠️ Duplicate Name Found',
        description: 'Same name is present in multiple person types. Please search by email to find the exact person.',
        variant: 'default',
      });
    }
    ...
    <>
      ...
      <Card className="hidden sm:block card-gradient">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button onClick={() => navigate(`/people/add?type=${type}`)} className="btn-blue">
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => navigate(`/people/edit/${person.id}?type=${type}`)}
                            >
      ...
      <div className="sm:hidden space-y-4">
        {loadingMap[type] ? (
          ...
          <div className="text-center text-muted-foreground">No {type}s found</div>
        ) : (
          filteredPeople.map((person) => (
            <Card key={person.id} className="p-4 rounded-xl border border-slate-200">
              <div className="text-sm space-y-2">
                ...
                <div><strong>PAN:</strong> {person.panNumber}</div>
                <div>
            ...
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                {loadingMap[key] ? (
                  ...
                  <>
                    <div className="text-lg font-bold">{peopleData[key]?.length || 0}</div>
